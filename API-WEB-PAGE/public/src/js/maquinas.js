              ...
              if (resposta2.ok) {
                resposta2.json().then(function (json2) {
                  ...
                  if (
                    ...
                    table_row.innerHTML += `<div class="machine-card" onclick="openDashboard(${
                      json2.id_maquina
                                ...
                                <div class="card-content">
                                    <span id='span_cpu'>CPU: ${Math.round(
                                      json2.uso_processador
                                    )}%</span>
                                    <span id='span_ram'>RAM: ${Math.round(
                                      porcentagem_ram
                                    )}% </span>                                 
                                   <span id='span_disco'>Disco:${Math.round(
                                     porcentagem_memoria
                                   )}%</span>
                                    <span> Status:${
                                      json2.status_web != null
                                        ? json2.status_web
                                        : " indisponivel"
                                    }</span>
                                </div>
                    ...
                    json2.want_ram == 0
                      ? (span_ram.style.display = "none")
                      : (span_ram.style.display = "block");
                    json2.want_cpu == 0
                      ? (span_cpu.style.display = "none")
                      : (span_ram.style.display = "block");
                    json2.want_disco == 0
                      ? (span_disco.style.display = "none")
                      : (span_ram.style.display = "block");
